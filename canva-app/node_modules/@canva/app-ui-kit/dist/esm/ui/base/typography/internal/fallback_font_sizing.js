...
export function useScaledFallbackFontStyles({ componentType, fontWeight: fontWeightNameOrNumber, textContent }) {
    let enableScaling;
    let fontStackMode;
    let fontStack;
    switch(componentType){
        ...
        case 'none':
            ...
            break;
        default:
            throw new UnreachableError(componentType);
    }
    const contentString = React.useMemo(()=>enableScaling ? stringForSizingWithoutDiacritics(textContent) : '', [
        enableScaling,
        textContent
    ]);
    const fontSize = 16;
    ...
    const isFontAvailable = (f)=>{
        switch(f.sourceType){
            ...
            case 'local':
                return true;
            default:
                throw new UnreachableError(f.sourceType);
        }
    };
...
function stringForSizing(node) {
    ...
    if (React.isValidElement(node) && node.props.children) return stringForSizing(node.props.children);
    return '';
}
function useFontLoadStatuses({ fontStack, text, fontSize, fontWeight }) {
    const [tasks, setTasks] = React.useState(()=>new Map());
    const createTaskKey = React.useCallback((font)=>{
    ...
    const getTask = React.useCallback((fontFamily)=>{
    ...
    const upsertTask = React.useCallback((fontFamily, task)=>{
        setTasks((prevTasks)=>{
...
class FontFamily {
    ...
    constructor(postscriptName, sourceType, charAspectRatios){
        ...
        Preconditions.checkArgument(charAspectRatios.has('0'));
    }
}
