...
function useFlyoutMenuContext(store) {
    return {
        store: _react.useMemo(()=>store ?? new FlyoutMenuStore(), [
            store
        ]),
...
function useNavigableRef({ disabled } = {}) {
    ...
    const setRef = _react.useCallback((el)=>{
        if (flyoutMenuStore?.itemRefs == null) return;
...
function useFlyoutMenuKeyboardNavigation({ store, onExit }) {
    return (0, _use_keyboard_navigation.useKeyboardNavigation)(store.itemRefs.size, {
        ...
        onChange: _react.useCallback((index)=>{
            const it = store?.itemRefs.values();
            let ref;
            for(let i = 0; i <= index; i++)ref = it?.next().value;
            ref?.focus();
            store.closeCurrentlyOpenMenu?.();
        }, [
            store
        ]),
        onExit
...
function useFlyoutMenuHover({ store, isOpen, close }) {
    ...
    const ref = _react.useCallback((el)=>{
        ...
        el?.addEventListener('mousemove', (e)=>{
            const menuBounds = store.currentlyOpenMenu?.getBoundingClientRect();
            const position = {
                x: e.clientX,
                y: e.clientY
            };
...
function useNestedTriggerHover({ isOpen }) {
    ...
    const ref = _react.useCallback((handle)=>{
        (0, _handle.dangerouslyGetHTMLElement)(handle)?.addEventListener('mousemove', (e)=>{
...
function byDocumentPosition(a, b) {
    ...
    if (!aElement?.isConnected || !bElement?.isConnected) return 0;
    return bElement && aElement.compareDocumentPosition?.(bElement) & Node.DOCUMENT_POSITION_PRECEDING ? 1 : -1;
}
