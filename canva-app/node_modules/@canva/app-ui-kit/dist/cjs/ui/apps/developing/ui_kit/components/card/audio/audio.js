...
const AudioCard = (props)=>{
    return (0, _jsxruntime.jsx)(_draggable.Draggable, {
        ...props,
...
function InternalAudioCard({ ariaLabel, audioPreviewUrl, durationInSeconds, title, description, content, thumbnailUrl, bottomEnd, bottomEndVisibility, topEnd, topEndVisibility, onDragStart, onDragEnd, onClick, loading, disabled, selectable, selected, onAudioLoad, onImageLoad, onTimeUpdate, onPlay, onPause, onEnded, ref }) {
    ...
    _react.useEffect(()=>{
        ...
        audio.current.onerror = ()=>{
            if (audio.current?.src === audioPreviewUrl) {
                setDuration(0);
                setCanPlay(false);
                setCurrentTime(0);
            }
        ...
        audio.current.onplay = ()=>{
            onPlay?.(audio.current?.currentTime ?? 0);
        };
    ...
    const playAudio = _react.useCallback(()=>{
    ...
    const toggleAudio = _react.useCallback((event)=>{
        ...
        if (audioContext.isPaused) audioContext.resume();
            ...
            audioContext.pause();
            onPause?.(audio.current?.currentTime ?? 0);
        }
    }, [
        audioContext,
        playAudio,
