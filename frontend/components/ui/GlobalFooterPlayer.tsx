...
export function GlobalFooterPlayer() {
    ...
    const { currentPreviewTrack, setPreviewTrack, isPlaying, setIsPlaying, rightPanelWidth, sidebarWidth } = useDAWStore();
    ...
    return (
        <div
            className="h-24 bg-[#0A0A0C] border-t border-[#1A1A1A] z-[100] flex items-center transition-all duration-300 relative overflow-hidden"
            style={{
                width: `calc(100% - ${rightPanelWidth || 0}px)`
            }}
        >
            <audio
                ref={audioRef}
            ...
            {/* Workspace Centering Wrapper */}
            <div className="flex-1 flex items-center h-full relative">
                {/* Workspace Offset Spacer (matches page sidebar) */}
                <div
                    style={{ width: `${sidebarWidth || 0}px` }}
                    className="shrink-0 transition-all duration-300 h-full"
                />

                {/* Interactive Area (Flex Sandwich for centering) */}
                <div className="flex-1 flex items-center justify-between h-full px-6">
                    ...
                    {/* Player Controls (Center) - Centered in Workspace */}
                    <div className="flex flex-col items-center gap-1.5 flex-1 justify-center z-10 px-4">
