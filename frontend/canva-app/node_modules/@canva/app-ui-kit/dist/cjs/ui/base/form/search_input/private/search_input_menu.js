...
function SearchInputMenu(props) {
    const { children, detachContent = false, border, onOutsidePointerDown, end, className, error, type = 'search', usePopover = false, captureFocus, ref, ...searchInputProps } = props;
    ...
    const onTabOutside = _react.useCallback((event)=>{
        ...
        if (event.key === 'Tab') setTimeout(()=>{
            if (document.activeElement instanceof HTMLElement && !outerRef.current?.contains(document.activeElement)) handleOutsideInputInteraction();
        }, 5);
    }, [
