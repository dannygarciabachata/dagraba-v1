...
const getSwatchContent = ({
  ...
  if (colors == null || colors.every(c => c === undefined || typeof c === 'string')) {
    const inlineStyles = getInlineStyles(colors, width ?? 0);
...
function Swatch({
  ...
  const areNoColorsApplied = colors?.length === 0;
  const isAtleastOneColorTransparent = !areNoColorsApplied && colors?.some(c => !c) || false;
  const areAllColorsTransparent = !areNoColorsApplied && colors?.every(c => !c) || false;
  ...
  const tooltipLabelText = !disableTooltip ? tooltipLabel ?? nameLabel ?? defaultLabel : '';
  const tooltipDescriptionText = tooltipDescription ?? (nameLabel ? defaultLabel : undefined);
  const ariaLabelText = ariaLabel ?? defaultLabel;
  ...
  _react.useImperativeHandle(ref, () => ({
    focus: () => {
      (clickableRef.current ?? deletableRef.current ?? staticRef.current)?.focus();
    },
    blur: () => {
      (clickableRef.current ?? deletableRef.current ?? staticRef.current)?.blur();
    }
  }));
...
const AddColorButton = props => {
  ...
  return (0, _jsxruntime.jsx)(Swatch, {
    ...
    tooltipLabel: tooltipLabel ?? _swatchmessages.SwatchMessages.addColor(),
    ariaLabel: ariaLabel ?? _swatchmessages.SwatchMessages.addColor(),
    Icon: Icon
  });
};