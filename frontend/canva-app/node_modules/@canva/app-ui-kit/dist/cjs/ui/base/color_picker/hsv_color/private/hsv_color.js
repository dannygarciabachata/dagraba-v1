...
Object.defineProperty(exports, "HsvColor", {
    enumerable: true,
    get: function() {
        return HsvColor;
    }
...
class HsvColor {
    static fromHexString(hex, alpha) {
        ...
        const rgb = _color.RgbColor.fromHexString(hex);
        const r = rgb.r / 255;
        const g = rgb.g / 255;
        const b = rgb.b / 255;
        const max = Math.max(r, g, b);
    ...
    with(channels) {
        return new HsvColor(channels.h ?? this.h, channels.s ?? this.s, channels.v ?? this.v, channels.alpha ?? this.alpha);
    }
    ...
    toRgba() {
        ...
        return new _color.RgbaColor(r, g, b, this.alpha);
    }
    equals(that) {
    ...
    constructor(h, s, v, alpha = 1){
        ...
        this.v = v;
        this.alpha = alpha;
        _preconditions.Preconditions.checkArgument(0 <= s && s <= 1, '0 <= saturation <= 1: {}', s);
        _preconditions.Preconditions.checkArgument(0 <= v && v <= 1, '0 <= value <= 1: {}', v);
