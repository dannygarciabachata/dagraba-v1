...
Object.defineProperty(exports, "BaseComboSelect", {
    enumerable: true,
    get: function() {
        return _BaseComboSelect;
    }
});
...
class BaseComboSelectStore extends _base_select_presenter.BaseSelectStore {
    static _makeObservable(instance) {
        (0, _make_observable.makeObservable)(instance, {
            value: _mobx.observable.ref,
            filteredOptions: _mobx.observable.ref
        });
    }
    constructor(...args){
        super(...args), this.value = (BaseComboSelectStore._makeObservable(this), undefined), this.filteredOptions = [];
    }
}
class BaseComboSelectPresenter extends _base_select_presenter.BaseSelectPresenter {
    static _makeObservable(instance) {
        (0, _make_observable.makeObservable)(instance, {
            filterOptions: _mobx.action
        });
    }
...
const defaultFilterFn = (value, options)=>{
    ...
    return value ? options.filter((option)=>{
        const lowerCaseOption = `${option.value ?? ''} ${option.label}`.toLocaleLowerCase();
        if (lowerCaseOption === lowerCaseValue) return false;
        return lowerCaseOption.replace(/[^\w]/g, '').includes(lowerCaseValue);
    }) : [];
};
let _BaseComboSelect;
new (_class = class extends _identity {
    constructor(){
        super(_BaseComboSelect), _initClass();
    }
}, (()=>{
    class BaseComboSelect extends (_React_Component = _react.Component) {
        static _makeObservable(instance) {
            (0, _make_observable.makeObservable)(instance, {
                onInputChange: _mobx.action
            });
        }
        get value() {
        ...
        render() {
            const { value, placeholder, onChange, ...props } = this.props;
        ...
        constructor(...args){
            super(...args), this.presenter = (_BaseComboSelect._makeObservable(this), new BaseComboSelectPresenter()), this.store = new BaseComboSelectStore(), this.onInputChange = (value, e)=>{
                const { filterFn = defaultFilterFn } = this.props;
                const options = (0, _base_select_util.flattenOptions)(this.props.options);
    ...
    ({ c: [_BaseComboSelect, _initClass] } = _apply_decs_2203_r(BaseComboSelect, [], [
