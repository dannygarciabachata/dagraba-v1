...
function useFocusableChildren(options) {
    ...
    const update = (0, _react.useCallback)(()=>{
        ...
        const focusables = getFocusableChildren(ref.current, options?.onlyTabbables);
        setFocusableChildren(focusables);
    }, [
        ref,
        options?.onlyTabbables
    ]);
...
function useKeyboardFocusNavigation() {
    ...
    const moveFocusBy = (0, _react.useCallback)((increment)=>{
        const focusedIndex = focusableChildren.findIndex((x)=>x?.matches(':focus'));
    ...
    return {
        ref: (0, _compose_refs.composeRefs)(keyCombinationHandlerRef, focusableChildrenRef)
    };
}
