...
_export(exports, {
    ...
    get toEaselDirection () {
        return toEaselDirection;
    }
});
...
const toEaselDirection = (direction)=>{
    if (typeof direction === 'string' && direction.toUpperCase() === 'RTL') return 'RTL';
    return 'LTR';
};
const internalSdkWindow = window;
const uiKit = internalSdkWindow?.['__canva__']?.['uiKit'];
uiKit?.registerAppUiKitVersion({
    version: _version.LATEST_VERSION
}).catch(()=>{});
function AppUiProvider({ children }) {
    const [theme, setTheme] = _react.useState();
